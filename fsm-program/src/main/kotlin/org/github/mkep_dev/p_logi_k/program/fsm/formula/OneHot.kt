...
/**
 * *OneHot* Operator with multiple arguments.
 * Representation of an operand that is itself a combination of two operands and an operator.
 *
 * This equals to "(...((element_0 **xor** element_1) **xor** ...) **xor** element_n)".
 *
 * @param operands the operands that are part of this operator. The list must contain at least one operand.
 */
class OneHot(operands: List<Operand<Boolean>>) : MultiArgumentOperator<Boolean, Boolean>(operands, Boolean::class) {
    override val operationString: String = "oneHot"

    override fun computeValue(valueProvider: (NamedDirectValueProvider<out Any>) -> Any): Boolean {
        return operands.count { it.computeValue(valueProvider) } == 1
    }
}